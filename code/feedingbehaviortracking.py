            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        #                                        bbox_top, bbox_w, bbox_h, x1, h, -1, i))
                        if save_txt:
                            # to MOT format
                            bbox_left = output[0]
                            bbox_top = output[1]
                            ...
                            if 370 < x1 < 2100 and y1 > 1000:
                                h = -3.51824203e-08 * x1 ** 3 + 1.00186921e-04 * x1 ** 2 -5.46056262e-02  * x1 + 1.19857094e+03

                                # Write MOT compliant results to file
                                with open(txt_path + '.txt', 'a') as f:
                                    f.write(('%g ' * 11 + '\n') % (total_processed_frames["三分之一帧数"],
                                    frame_idx + 1, id, bbox_left, bbox_top, bbox_w, bbox_h, y1, h, cls, i))
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox/seg to image
                            ...
                            if 370 < x1< 2100 and y1>1000:
                                h=-1.63190408e-08* x1** 3 +9.93639926e-06* x1 ** 2+  6.86557282e-02 * x1 + 1.16340450e+03
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=Path, default='ubantu4.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--source', type=str, default='D:\\cow video\\4\\am\\1',
    ...
    parser.add_argument('--project', default=ROOT / 'runs' / 'track4test', help='save results to project/name')
...
def main(opt):
    ...
    #run(**vars(opt))
 # 查找文件夹中的所有视频文件
    video_files = glob.glob(opt.source + '*.mp4')
